...
> lint / types / RLS / review). This file explains the architecture and the *why*; `RULES.md`
> states the *what*. Where the two appear to disagree, `RULES.md` wins and the drift should be
> fixed.
...
- **Never leave a dependency bump uncommitted** — `package.json` reads as one modified file but
  ...
  `npx tsc` here ([`RULES.md` §9](RULES.md)).
