...
**Error shaping:** data functions return discriminated snapshots (see `BatchesSnapshot`: `ok` / `sync-failed` / `unconfigured`) so Server Components map states straight to UI. Neither `unconfigured` (no Supabase env) nor `sync-failed` (configured but errored) may substitute mock or fabricated data — both render an honest empty state; `sync-failed` must additionally surface the sync-failed banner. Never leak raw Supabase/SQL errors, table names, or internal IDs to the UI.
