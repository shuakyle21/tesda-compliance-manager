...
description: "Reference for the TVI-CAMS Supabase schema — live 18 tables / 36 FKs / seven enums, checked-in target 25 tables / 52 FKs / eight enums once the pending ADR-001 billing domain lands — the seven-migration ledger (four applied and recorded, one applied by hand, one missing on live, one pending) with the #230 repo-vs-database drift, the per-table RLS policy map including the seven billing-domain tables, storage policies for the private compliance-evidence bucket, the ADR-006 school registry and platform-admin boundary, profile provisioning, the database.types.ts regeneration contract, anon-key client wiring, and RULES section 10 agent-conduct guardrails."
...
The checked-in migration history is the only trustworthy description of the schema. [`docs/DATA_MODEL.md`](/docs/DATA_MODEL.md) is its status ledger, and its version table (re-verified against the live project on **2026-09-10** by catalog query — `pg_proc`, `pg_policies`, not inference) is the source for applied vs pending below — with two caveats: the table still lists only **seven** versions (the same-PR ledger update for row 8 that the file's own maintenance rule requires has not landed, so the migrations directory is ahead of its own ledger), and the ledger's statuses are *object* statuses, not migration-table statuses: row 5 is applied by hand and unrecorded, row 6 is genuinely missing, and row 4 has moved to `supabase/seeds/` with its rows still live (see below). Applied vs pending is therefore read from the ledger plus the migration header notes, never by querying the live project (RULES §10):
...
| 4 | `20260831120000` | [`seed_dev_operational_data.sql`](/supabase/seeds/20260831120000_seed_dev_operational_data.sql) | **Moved to `supabase/seeds/`** — a dev fixture, no longer a migration. Seeds **dev operational data**: five `DEV-`-prefixed batches, placeholder learner rosters, per-batch document rows, and the `documents_batch_id_document_key_key` unique index. Data only, no DDL. | **not recorded; rows present on live** |
...
| 5 | `20260904120000` | [`add_user_admin_write_policies.sql`](/supabase/migrations/20260904120000_add_user_admin_write_policies.sql) | Four admin-only write policies that make [`/users/new`](/app/(dashboard)/users/new/page.tsx) executable. Policies only, no DDL. | **applied by hand, not recorded** |
| 6 | `20260906120000` | [`ensure_invitation_membership_atomic.sql`](/supabase/migrations/20260906120000_ensure_invitation_membership_atomic.sql) | `public.ensure_profile_tenant_membership(...)` — the atomic invitation-membership function used by the Clerk webhook. One function, no DDL. | **missing on live** (not applied, not recorded) |
| 7 | `20260906114735` | [`add_school_registry_and_platform_admin.sql`](/supabase/migrations/20260906114735_add_school_registry_and_platform_admin.sql) | **ADR-006**: three tables (`qualifications`, `tenant_qualifications`, `platform_admins`), six new `tenants` columns, `app_private.is_platform_admin()`, `public.current_user_is_platform_admin()`, `public.create_school(...)`, the platform-admin policies, and an idempotent six-qualification seed. | applied |
...
**Applied out of order — and the checked-in history and the live schema currently differ.** `20260906114735` was applied on 2026-09-06 while `20260831120000`, `20260904120000`, and `20260906120000` were still unapplied — it depends on none of them, only on the base schema. Supabase records migrations by version, so applying the earlier ones later is fine; **just do not assume "highest applied version" means everything below it has run.** As things stand, the live database's migration table holds exactly the four applied-and-recorded rows above — not all seven files in `supabase/migrations/` (plus the moved seed in `supabase/seeds/`) — so read the Status column, not the version numbers, when asking what the running schema contains. Each row was re-verified on 2026-09-10.

**#230: the repo and the live database have drifted (verified 2026-09-10).** The live project's migration table returns four versions: the three applied base ones plus the school registry — recorded under **`20260906114735`**, and the file has since been **renamed to match** (it was named `20260906130000` — the same migration under two versions; all ten of its policies plus `app_private.is_platform_admin()` were verified present before the rename, so that part of the drift is closed). Two checked-in objects exist on live but are not recorded in the migration table: the dev fixture's rows (row 4 — batches/learners/documents counts of 5/89/40 match its content exactly) and the four admin write policies (row 5, applied by hand). The verification recorded in the 8th migration's header, in [docs/DEMO_DATA_VALIDATION_PLAN.md](/docs/DEMO_DATA_VALIDATION_PLAN.md) Phase 0, and in the re-verified ledger found the database **ahead of its own records rather than behind them** — and the one real gap is row 6: **`public.ensure_profile_tenant_membership`** does not exist on live (not applied, not recorded) even though [`modules/auth/data/provisioning.ts`](/modules/auth/data/provisioning.ts) calls it. Two consequences: the `documents_batch_id_document_key_key` unique index is of unknown existence (any new seed using `on conflict (batch_id, document_key)` fails without it), and **reconcile #230 before assuming a clean `db push`** — the 8th migration depends on nothing of that reconciliation (every table and function it references was verified present), but the push mechanics need sorting out first.

**The behaviour-only rows — and what the re-verification settled.** Row 5 (`20260904120000`) is now recorded as **applied by hand, not recorded**: its four policies are present on live, so the direct path is not RLS-denied today and `/users/new` can assign — the two screens' pairing concern (a brand-new school's first admin) now rests only on row 6. What is genuinely missing on live is `ensure_profile_tenant_membership`: until the function exists, the membership half of an invitation grant cannot land, and the webhook rethrows (500) so Clerk keeps retrying — the profile row itself still lands, because the webhook runs on the service-role client. Row 4 (`20260831120000`) seeds the dev operational rows the dashboard and the isolation assertion need — present on live but not recorded in the migration table, and its file has moved to `supabase/seeds/` so a future `db push` cannot re-apply it (the rows stay; what to do with them is tracked on #230).

**The only pending migration — the newest — is a shape change.** [The ADR-001 billing domain](#the-adr-001-billing-domain-migration-8-pending) adds seven tables, 16 foreign keys, columns on three existing ones, an eighth enum, a widened RLS helper, and a storage DELETE policy; it is unapplied and unexecuted as of writing.
...
Migration 7 ([`20260906114735`](/supabase/migrations/20260906114735_add_school_registry_and_platform_admin.sql)), implemented by [ADR-006](/docs/adr/ADR-006-platform-admin-and-school-registry.md) (accepted 2026-09-06, supersedes the PRD's FR-02 "Super Admin is not implemented" prohibition), closes the hole the user screen left: `/users/new` grants a person access to a school that **already exists**, and nothing created the school — the three tenants were inserted by hand at the bottom of the canonical migration, so onboarding a new TVI meant editing SQL. (Record-keeping note: the live migration table records this migration as `20260906114735`; the file was originally named `20260906130000` and has since been **renamed to match**, after all ten of its policies and `app_private.is_platform_admin()` were verified present — see the #230 note in [Migration history](#migration-history).)
...
**Nothing reads these tables yet.** No application code fetches or writes any of the seven tables: [`modules/billing/data/billing.ts`](/modules/billing/data/billing.ts) derives billing cards entirely from the batch plus the document matrix — no live per-program source is wired yet (TES-34), so nothing reaches the new tables; [`packets.ts`](/modules/billing/domain/packets.ts) still carries its "table not yet migrated" comment, and the packet's `generated` state stands in from batch state (the ADR-002 prototype portrayal, completed batches as `settled`). The data-layer wiring and the `database.types.ts` regeneration (IMPLEMENTATION_PLAN Phase 0.2, which depends on Phase 0.1 applying) both follow the apply.
...
The policy sections across the seven checked-in migrations give the following map. Unmarked rows are the applied base; **(7)** rows are applied with ADR-006 (file renamed `20260906114735` to match its recorded version); **(5)** rows are applied on live by hand — not recorded in the migration table, per the 2026-09-10 re-verification, so those cells are populated; **(8)** rows are checked in and pending with the ADR-001 billing domain — until migration 8 runs, none of the seven new tables exists on live at all:
...
- **A hazard closure:** `documents` had no uniqueness on `(batch_id, document_key)`, so any repeated insert silently duplicated rows. The fixture adds the **unique index** `documents_batch_id_document_key_key` — it fails loudly if duplicates already exist (de-duplicate first), and being an index rather than a constraint it produces no `database.types.ts` change.
...
**`seeds/verify_user_admin_setup.sql` (admin-policy verification setup).** A pasteable, two-part setup in **one all-or-nothing transaction** for making the user-creation screen (PR #213 / commit `b3361a6`) observable at runtime — a **seed, not a migration**, because part 2 hardcodes a Clerk user ID (different between Clerk dev and prod instances, the same reason as `dev_profile_memberships.sql`). It exists because the ledger then recorded migration 5 as pending on the live project while demo is deliberately a `viewer`, so `/users/new` renders its admin-only guard and neither direct write path is reachable. **The 2026-09-10 re-verification records the row-5 policies as applied on live by hand (not recorded in the migration table), so that premise is stale — verify the live state before running the seed.**
...
- **No `database.types.ts` impact:** policies only, no tables/columns/enums — the regeneration contract does not fire; what changes on landing is the `(5)` cells of the [policy map](#per-table-policy-map) and the `20260904120000` row of `docs/DATA_MODEL.md` (now recorded as applied, not recorded).
...
Consequently: **answer schema questions from the checked-in migrations, `supabase/seeds/`, and `lib/supabase/database.types.ts` first** — with the standing caveat that the checked-in history and the live schema currently differ: the live migration table records four versions only, the re-verified ledger carries row 5 as applied-by-hand and unrecorded, row 6 (the membership RPC) as genuinely missing, row 4's dev fixture as moved to `supabase/seeds/` with its rows still live, and the school registry was applied out of order and is now named to match its recorded version `20260906114735` (see the [Migration history](#migration-history) status table and its #230 note). The read-only introspection tools (`list_tables`, `list_migrations`, `get_advisors`, `search_docs`) remain available for what those cannot answer — with the caveat that `list_tables` row counts are `reltuples` planner estimates, not counts: the 8th migration's seed comment records exactly this trap, where an earlier draft called `scholarship_programs` empty from the estimate until a real `count(*)` corrected it.
...
- [Quickstart and Task Routing](/openwiki/quickstart.md) — where to start, the env vars that decide `ok` versus `unconfigured`, and the current known states a schema change must account for (seven migrations: four applied and recorded, the write policies applied by hand and unrecorded, the dev fixture moved to `supabase/seeds/` with its rows still live, the school registry renamed to match its recorded version, the membership RPC missing; reconcile #230 before any `db push`)
...
- [`RULES.md`](/RULES.md) §4 (rules 21–26) — the design-system rules the RLS zero-row and `sync-failed` paths render into: every data screen implements all six states (loading, empty, no-results, error/sync-failed, permission-denied, stale-data) and relative dates carry a "Data as of" stamp (rule 24)
